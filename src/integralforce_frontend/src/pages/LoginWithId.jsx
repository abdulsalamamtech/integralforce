...
import { createActor } from '../../../declarations/integralforce_backend';
import { canisterId } from '../../../declarations/integralforce_backend/index.js';
...
const LoginWithId = () => {
  ...
  const { user, logout: icpLogout, login: icpLogin } = useUser();
  ...
  const login = async () => {
    ...
    // Get the user's identity after successful login
    const identity = authClient.getIdentity();
    console.log('User identity:', identity);
    icpLogin(identity.trim());
    navigate('/dashboard');
  };
  ...
  return (
    <div className="flex items-center justify-center bg-gradient-card p-6 min-h-fit">
      ...
      <div className="w-full md:w-[70%] p-8 text-card-foreground bg-opacity-10 backdrop-blur-lg shadow-xl rounded-2xl border border-white/20">
          <h1 className="text-3xl font-bold text-gray-700 text-center mb-2">Who Am I?</h1>
